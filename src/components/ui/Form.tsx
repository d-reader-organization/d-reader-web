...
>(({ className, ...props }, ref) => {
  ...
  return (
    <Label
      ref={ref}
      className={cn('text-base', error && 'text-red-500 dark:text-red-900', className)}
      htmlFor={formItemId}
      {...props}
    />
  )
...
const FormDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <p ref={ref} id={formDescriptionId} className={cn('text-sm text-grey-100 leading-4', className)} {...props} />
    )
  }
)
